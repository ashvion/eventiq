...
This project is configured for easy deployment on **Vercel** with a persistent database.

### 1. Database Setup
Register for a free **Vercel Postgres** or **Neon.tech** database and get your `DATABASE_URL`.

### 2. Environment Variables
Set the following variables in Vercel:
- `GEMINI_API_KEY`: Your Google Gemini API Key.
- `DATABASE_URL`: Your Postgres connection string.
- `DJANGO_SECRET_KEY`: A random strong string.
...
### 4. Create Production Superuser (Option A)
Once your site is live and the database is connected, run these commands in your local terminal to gain admin access:
